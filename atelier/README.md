...
| **Veillée** | Le run du jour : un arbre de 64 feuilles WOTS+ pour vie, un geste = une feuille, un coup = une feuille, 9 salles tirées du premier bloc du jour, la salle tenue et sa bataille, preuve `eidos-veillee/2` jugée sans rejeu. |
| **Guide** | Mode d’emploi. |
...
| **Vigil** | The daily run: a tree of 64 WOTS+ leaves as life, one gesture = one leaf, one blow = one leaf, 9 rooms from the day's first block, the held room and its battle, an `eidos-veillee/2` proof judged without replay. |
| **Guide** | How to use it. |
