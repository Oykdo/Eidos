...
| **Veillée** | Le run du jour : un arbre de 64 feuilles WOTS+ pour vie, un geste = une feuille, 9 salles tirées du premier bloc du jour, preuve `eidos-veillee/1` jugée sans rejeu. |
| **Guide** | Mode d’emploi. |
...
| **Vigil** | The daily run: a tree of 64 WOTS+ leaves as life, one gesture = one leaf, 9 rooms from the day's first block, an `eidos-veillee/1` proof judged without replay. |
| **Guide** | How to use it. |
