...
|---|---|
| Émission cumulée | **944,265 825 16 IONOS** |
...
| Budget robinet de l'époque servi | 1 / 5 040 IONOS = **0,019 8 %** ; **un seul compte** depuis la genèse |
| Récompense au bloc 45 | 21,249 150 83 IONOS (creux du cosinus ; moyenne d'époque : 40) |
| Créneaux pourvus | 46 / 100 — une panne contiguë (créneaux 8→61, 54 h), **38/38 depuis** |

**Le mot « circulation » d'`etat.json` ne mesure rien.** L'invariant impose `Σ utxo == emission_cumulee` : `en_circulation_atomes` vaut toujours l'émission, trésor compris. La grandeur qui manque est **le hors-trésor**, soit 1,00 IONOS. Toute la suite se lit sur celle-là.

**Projection à robinet inchangé** (1 IONOS par compte et par époque de 42 jours, un compte connu) : h = 1 045 (42 j) → 41 094 IONOS dormants sur 41 095 émis ; h = 10 045 (417 j) → 402 517 sur 402 518 ; h = 100 045 (11,4 ans) → la quasi-totalité de 3 998 873.

Même saturé, le robinet ne renverse pas cela : `MAX_PAIEMENTS = 3` par bloc et les paiements ne sont construits qu'au **dernier bloc de chaque exécution** du cron, soit 3/heure → 3 024 IONOS par époque, **60 % seulement de son propre budget** `a·T/8 = 5 040`. Les 7/8 dorment, et le huitième restant n'est atteignable qu'aux trois cinquièmes.
...
Un créneau = 3 600 s, donc **12 h = 12 blocs**. `1008 / 12 = 84` exactement, et chaque âge compte un nombre entier d'époques : une fenêtre de 12 blocs ne chevauche jamais deux âges. Émission réelle d'une fenêtre de 12 blocs à Satya : **min 240,20 / moyenne 480,00 / max 719,80**. Indexer le gain sur le maximum instantané le ferait dépendre de la phase du cosinus ; on prend la moyenne, exacte à l'atome sur l'époque :
    ...
    plafond de la fenêtre = 12·a/8 = 3a/2 ionos     et     84 × 3a/2 = 126a = a·T/8

| Âge | a | plafond / 12 blocs | par bloc | × 84 sillons | durée de l'âge |
|---|---|---|---|---|---|
| Satya | 40 | **60 IONOS** | 5,00 | 5 040 = a·T/8 | 95,7 ans |
| Trétâ | 30 | **45** | 3,75 | 3 780 | 71,8 ans |
| Dvâpara | 20 | **30** | 2,50 | 2 520 | 47,8 ans |
| Kali | 10 | **15** | 1,25 | 1 260 | 23,9 ans |

`3a/2` est entier pour les quatre âges : aucun atome à répartir au bord. Sur les fenêtres déjà forgées (blocs 0–47, creux du cosinus), 60 IONOS valent 23,7 % à 24,8 % de ce que la fenêtre émet — le trésor engrange encore les trois quarts. Les trois autres âges sont hors d'atteinte de ce réseau d'essai (Satya dure 95,7 ans) ; la table existe pour que la formule ne dépende d'aucune constante d'âge.
...
- **Le reliquat ne se reporte pas.** Un sillon sans glaneur perd ses 60 IONOS : sinon un arrêt de 54 h (mesuré §1) armerait une rafale de 270 IONOS au redémarrage.
...
- `est_goutte(tx)` reconnaît une goutte à une sortie valant exactement 100 000 000 atomes : **une gerbe de 1 IONOS serait comptée dans `robinet_epoque_atomes`.** La marque doit être structurelle et rejouable — la monnaie rendue du robinet va sur `adr(sha256("<GRAINE>/rendu/<h>/<k>"))`, celle de la Moisson ira sur `adr(sha256("<GRAINE>/glane/<h>"))` : le rejeu distingue les deux canaux sans rien croire.
...
**Retenu : le verrou 3 du §6, et rien d'autre.** Un glaneur ne détient jamais plus d'une gerbe non dépensée. Accumuler *n* ionos coûte *n* transactions signées, *n* clés WOTS+ brûlées, 2*n* issues et 2 283 × *n* octets de chaîne : 10 IONOS = 5 jours et 20 issues ; 100 IONOS = 50 jours, 200 issues, 228 ko. L'accumulation stérile n'est pas interdite, elle est **facturée en clés** — et chaque dépense forcée est le moment exact où un puits peut prélever.

**Écarté : la gerbe qui décroît avec la bourse** (`G = G_max·2^−⌊solde/4⌋`, qui plafonnerait un thésauriseur à ≈ 34 IONOS au bout d'un an contre 730 pour un gain plat). Le nœud ne mesure la bourse que sur les adresses qu'il a servies ; une transaction vers une adresse fraîche la rend invisible. Une règle contournable pour une transaction est une taxe sur les honnêtes.

**Optionnel, quand les puits existeront : l'appariement.** `gerbe = socle (0,25) + min(0,75 ; ce que ce compte a versé à un puits depuis sa dernière gerbe)`. La boucle « verser puis récupérer » est alors **exactement neutre** : seul le socle grossit la masse, et l'inflation nette vaut 0,25 IONOS par glaneur actif et par sillon quoi que fasse le joueur. Capacité correspondante : 60 / 1,25 = 48 glaneurs au plafond.
...
|---|---|---|---|
| 0,50 | 120 IONOS | 2,0 | 1,0 j |
| 0,20 | 300 | 5,0 | 2,5 j |
| 0,10 | 600 | 10,0 | 5,0 j |
| 0,02 | 3 000 | 50,0 | 25 j |
| 0 | +43 830 IONOS/an, linéaire | — | asymptote 1/8 de l'émission |

**Débit de puits exigé au point fixe : `3a/2` par sillon — 60 IONOS à Satya, 5 par bloc, soit une gerbe par glaneur actif et par 12 h.** Le panier moyen d'un joueur actif doit donc coûter **une gerbe par sillon** (1 IONOS à Satya, 0,25 à Kali) : tarifé à 0,01 un puits ne stabilise rien, tarifé à 100 il vide les bourses en un sillon.
...
1. **La gerbe : 1 IONOS, ou 0,25 ?** À 1 IONOS le plafond porte 60 glaneurs et la gerbe est lisible (« un ionos, comme le robinet ») ; à 0,25 il en porte 240 (borné à 128 par la transaction) et la Sybil rapporte quatre fois moins par compte. Recommandation : **1 IONOS, prorata au-delà de 60 glaneurs** — la rareté se voit, la dilution est publiée.
