...
| 27 salles, 26 franchir | **9 salles, 8 franchir** — une par bande, une par muse | décision d'auteur, codée le 2026-09-16 ; `ETAGES_PAR_BANDE : 3 → 1` **ne suffisait pas** : l'étage se choisit désormais par le hachage dans le neuvième du cran (§6.1) |
...
### 4.2 Option A — la feuille signe un coup porté (ce que `bataille.ts` fait) [RETENUE par l'auteur le 2026-09-15, sur les mesures du §4.5]
...
### 4.3 Option B — la feuille signe une mort [NON RETENUE — tranché par l'auteur le 2026-09-15 : un coup, arbre de 64, §4.2 et §4.5]
...
Neuf salles, **une par bande, donc une par muse**, de Thalie (l'étage 0, la porte de la ville) à Uranie. C'est une décision d'auteur, **codée le 2026-09-16** (PR « neuf-salles ») : `pendule.ts` porte `ETAGES_PAR_BANDE = 1`, donc `ETAPES = 9`, et `veillee.FRANCHIR_AU_SOMMET = ETAPES − 1 = 8`.

**Ce que ça a changé : une constante, et une formule que la constante a rendue fausse.** `ETAGES_PAR_BANDE : 3 → 1` ; alors `ETAPES = 9`, `etageDe` prend `k = i` (la bande est l'étape) et `FRANCHIR_AU_SOMMET` suit à 8. Mais le décalage `⌊p·(taille − 1)/8⌋` n'offrait plus que **neuf étages par bande** — 73 sur 255, toujours les mêmes, la couverture de phase 0 tombait de 200 à 73. Depuis, `etageDe(i, p, h)` prend le hachage de la transition : le cran `p` choisit le **neuvième** de la bande (cran 0 en bas, cran 8 en haut), l'octet `h[1]` choisit l'étage dans ce neuvième — 227 étages joignables (la bande 0 n'a que sa porte, l'étape 0), les trois seuils de phase 0 tenus. Le pendule garde tout son rôle : sa position choisit la région, le hachage la salle. Le parcours reste celui du pendule-9 [FIXE] et la transition ne bouge pas (graine, étape, position, choix, objet porté, résonance de l'étage quitté, sens de la muse).
...
- **La sortie (V3) et les Indéchiffrés (V6) ne sont pas codés ; les neuf salles et la bataille dans la veillée le sont depuis le 2026-09-16** (C4 PR 5b : `eidos-veillee/2`, le geste `frapper`, la salle tenue, les abattus, la première tombée). Le code d'aujourd'hui fait 9 étapes, cinq gestes, un arbre de 64 feuilles et une feuille par coup porté (A17, tranché « un coup » : « une feuille, une mort » est écarté). Le juge ne rejoue pas encore la bataille (PR 6). Cette bible dit ce qu'il faut construire, pas ce qui existe : la marque d'état de chaque paragraphe est la seule chose qui engage.
