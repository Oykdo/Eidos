...
| Le budget d'une run entière, sous chaque règle | **1 000 runs × 39 configurations** (coup 64, mort 32, mort 64 × 27 ou 9 salles × roster qui revient ou permadeath, puis à 9 salles les neuf contreparties d'A28), sans signature, `ia.ts` des deux côtés, configurations appariées ; 27 salles : **0 ‰** d'arrivés sous toute règle ; 9 salles : 997–1 000 ‰, avec **23** feuilles de butin (« un coup », 64) ou **9** (« une mort », 32) ; permadeath telle qu'écrite : roster balayé à la salle 1 dans 996–1 000 ‰ des runs ; ses contreparties (2026-09-15) : « perdue » seule 230 ‰, recrue 120 ‰, réserve de 12 : 818 ‰ pour 8,45 objets par run, **« perdue » sur une réserve de 9 : 907 ‰ pour 3,69 objets**, une défaite qui ne coûte que la première tombée sur 6 objets : **995 ‰ pour 1,27** ; échantillon de 24 runs en CI en ~30 s | `atelier/scripts/banc-veillee.ts` (`npm run banc-veillee`), `banc-veillee.test.ts` |
