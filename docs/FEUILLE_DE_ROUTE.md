...
4. Ce qui manque n'est pas du code, c'est du **branchement et du prix** : le moteur tactique, `tiers.ts` et `lignee.ts` n'ont aucun importateur hors de leurs tests ; `reliques.json` et `veillees/index.json` sont vides ; l'ionos n'achète encore rien.
...
|---|---|---|
| Émission bornée, sans halving | 2 080 époques × 1008 = **2 096 640 blocs**, **62 899 200 IONOS**, 239,18 ans ; cosinus en `Decimal` par série de Taylor, π à 68 décimales, **aucun `math.cos`** | `verify_genesis.py` → 32 contrôles ; `eonis.py` → 6 ; `eonis.py` gelé (SHA `2eb70acb…` dans `genesis.json`) |
...
| Le budget d'une run entière, sous chaque règle | **1 000 runs × 45 configurations** (coup 64, mort 32, mort 64 × 27 ou 9 salles × roster qui revient ou permadeath, puis à 9 salles les neuf contreparties d'A28 et un bot qui ramasse un ou deux gestes par salle), sans signature, `ia.ts` des deux côtés, configurations appariées ; 27 salles : **0 ‰** d'arrivés sous toute règle ; 9 salles : 997–1 000 ‰, avec **23** feuilles de butin (« un coup », 64) ou **9** (« une mort », 32) ; permadeath telle qu'écrite : roster balayé à la salle 1 dans 996–1 000 ‰ des runs ; ses contreparties (2026-09-15) : « perdue » seule 230 ‰, recrue 120 ‰, réserve de 12 : 818 ‰ pour 8,45 objets par run, **« perdue » sur une réserve de 9 : 907 ‰ pour 3,69 objets**, une défaite qui ne coûte que la première tombée sur 6 objets : **995 ‰ pour 1,27** ; le bot qui ramasse : « une mort à 32 » finance **un** geste par salle (168 ‰ de refus, 105 ‰ d'arbres vides), « un coup à 64 » en finance **deux** (930 ‰ arrivent, 11 feuilles restantes) ; échantillon de 24 runs en CI en ~35 s | `atelier/scripts/banc-veillee.ts` (`npm run banc-veillee`), `banc-veillee.test.ts` |
...
|---|---|---|---|
| `SPEC_PUITS.md` | ce qui absorbe les ionos | **192 transactions par jour** pour tout le réseau (`MAX_ENVOIS = 8`) et 120 IONOS/jour ; une sortie coûte 28 o contre 2 283 o pour une transaction — **81,5×** ; point fixe **s = 0,1115 IONOS/joueur/jour**, soit 1 076 joueurs | 6 (§7) |
| `SPEC_MOISSON.md` | ce qui sort du trésor | le trésor garde **99,894 1 %** de l'émission ; hors-trésor à h = 45 : **1,00 IONOS**, un seul compte servi depuis la genèse | 3 |
...
`robinet.py:180` `auteur_autorise` n'est appelé qu'en `:310`, dans `ajouter()`. `ajouter_envoi()` (`:252`) n'a ni auteur, ni époque, ni quota : le seul plafond restant est `MAX_FILE = 200` en attente et `MAX_ENVOIS = 8` par bloc. C'est cohérent avec la docstring — le frein rationne les **ionos**, pas les **créneaux** — mais la ressource rare d'Eidos est le créneau : **192 transactions par jour**, et une goutte de robinet (1 IONOS) finance 99 999 999 transactions à 1 atome, soit **1 427 ans** de la capacité entière du réseau.
...
| **A5** | D5 — une unité tombée en bataille ancrée quitte-t-elle le roster ? | **TRANCHÉ PAR L'AUTEUR le 2026-09-14 : oui.** En veillée ancrée seulement ; en libre, rien (une lecture). C'est le seul puits réel du jeu. **Mesuré le même jour (banc 5a)** : telle qu'écrite, la règle vide le roster à la salle 1 en médiane dans 996 à 1 000 ‰ des runs — une bataille gagnée coûte aussi des unités ; elle exige une contrepartie, voir **A28** — tranchée le 2026-09-15 : la première tombée d'une bataille perdue | Sans elle, le marché de D6 n'a qu'une entrée ; avec elle, la permadeath devient une décision d'économie, pas de difficulté |
| **A6** | P1 — la mise du sceau se paie-t-elle par acte ou par mise ? | **La mise** (4 IONOS / 64 actes), en disant que le brûlage est la **preuve** et le décompte une **figure** | Un décompte hors chaîne dans `batailles/index.json` ; c'est la seule forme qui tienne dans 192 tx/jour |
...
| **A9** | La gerbe de la Moisson : 1 IONOS ou 0,25 ? | **1 IONOS, prorata au-delà de 60 glaneurs** | À 0,25 le plafond porte 240 glaneurs (borné à 128 par la transaction) et la Sybil rapporte 4× moins par compte ; à 1 IONOS la rareté se voit et la dilution est publiée |
...
|---|---|---|---|
| **A17** | en combat, une feuille signe un coup (`SPEC_TACTIQUE` D2, 64 feuilles) ou une mort (`BIBLE_VEILLEE` §4.3, 32) ? | `HANDOVER_VEILLEE_BATAILLE.md` §3 | **TRANCHÉ PAR L'AUTEUR le 2026-09-15 : un coup, arbre de 64** (D2 confirmée, rien au moteur), après avoir demandé si l'équilibre tactique mesuré à 64 feuilles par bataille (R2, `banc-r2`) tient dans une run où l'arbre se partage : oui — les duels de R2 s'ouvrent à 64 feuilles et en dépensent ≈ 4, la politique compte les feuilles (rang 3 d'`ia.ts`), et dans la run le plafond ne mord que dans 7 ‰ des batailles à deux gestes de butin par salle, 0 ‰ sans butin. Le chemin : **MESURÉ le 2026-09-14 (banc 5a, 1 000 runs × 12)** : à 9 salles les deux règles arrivent au bout (997–1 000 ‰) ; la différence est le **butin** — « un coup » à 64 laisse **23** feuilles (18–28) pour l'hôte, la bêche et la capsule, « une mort » à 32 en laisse **9** (7–11). Le seuil de la bible (≤ 80 % gardent > 6) classe tout « trop long » : c'est un choix d'appétit, **à l'auteur** — 23 gestes de butin pour 9 salles, ou 9. **Puis mesuré avec un bot qui ramasse (2026-09-15, six configurations)** sur la question de l'auteur (« sous une mort à 32, plus de chances de finir ? ») : **non** — sans butin les deux arrivent pareil ; à un geste par salle, 32 refuse un geste dans 168 ‰ des runs et se vide en bataille dans 105 ‰ (3 feuilles à l'arrivée), à deux gestes il se vide dans 658 ‰ ; 64 (un coup) finance un geste sans y penser (17 restantes) et deux presque toujours (930 ‰, 11 restantes). **32 finance un geste par salle, pas toujours ; 64 en finance deux.** Ce qui reste pour « une mort » : le prix d'une salle se connaît d'avance (ses occupants). Recommandation : « un coup à 64 », déjà en place |
| **A18** | 27 salles ou 9 ? | idem | **TRANCHÉ PAR LA MESURE le 2026-09-14 : 9.** À 27 salles avec une bataille par salle, aucune règle n'arrive (0 ‰, 0 ‰, 57 ‰ à 64 feuilles-morts : 26 batailles à 3,7 coups ou 1,7 mort) ; à 9, les trois arbres arrivent. **Confirmé par l'auteur le 2026-09-16 et codé** (PR « neuf-salles ») : `ETAGES_PAR_BANDE` 3 → 1, `SAC_PLACES = DALLE_N²` (81, neuf par salle). Découvert en codant : à un étage par bande, le cran seul n'offrait que 9 étages par bande — 73 sur 255, toujours les mêmes, et la couverture de phase 0 tombait à 73 (seuil 200) ; `etageDe(i, p, h)` prend désormais le hachage de la transition (h[1]) : le cran choisit le neuvième de la bande, le hachage l'étage dedans — **227 étages joignables** (la bande 0 n'a que sa porte), répétition 0,139, butin par ligne 0,119. Les six configurations qui ont fondé A17 et A28 rejouées à 1 000 runs sur ce monde : mêmes verdicts, chaque chiffre dans les tolérances (coup-64-9 : 23 feuilles restantes en médiane, perdue-1 : 496 ‰ d'arrivées, b2 : tient). Et sans bataille, la Tour seule ne vide plus l'arbre (≤ 47 feuilles sur 64) : le dilemme de la dernière feuille arrive avec 5b |
| **A19** | d'où le juge tire-t-il le roster d'une bataille rejouée ? | idem §6 | l'auteur, avant PR 6 |
| **A28** | la permadeath vide le roster (A5 mesuré) : quelle contrepartie ? (a) une unité tombée ne compte que dans une bataille **perdue** ; (b) le recrutement en cours de run — les captures se battent ; (c) assumer des runs de deux salles ; (d) le coffre porte une réserve, le jeu tel que codé ; (e) une défaite ne coûte que la première tombée | `BIBLE_VEILLEE.md` §4.5, V5 | **TRANCHÉ PAR L'AUTEUR le 2026-09-15 : (e), la première tombée** — en veillée ancrée, une bataille perdue retire du coffre la première unité tombée (la cible du premier coup du journal qui retire une unité du coffre, riposte comprise) ; gagnée ou nulle, rien ne quitte le coffre ; la réserve est le coffre tel qu'il est (`MAX_COFFRE` borne la lice). Mesuré : 995 ‰ au bout et 1,27 objet par run à six objets, 460 ‰ à trois. Le chemin : **MESURÉ le 2026-09-15 (banc 5a, 21 configurations de plus, 1 000 runs × 9 salles)** : (a) seule, 230 ‰ d'arrivées — une run s'arrête à sa première défaite, et 77 % en ont une, à la salle 0 ; (b) à son plafond (capsule gratuite, capture sûre), **120 ‰** : un chantier que sa borne haute tue ; (d) seule mange ≈ 1 objet par bataille, 12 objets pour 818 ‰ et **8,45** brûlés par run ; **(a) + (d)** : perdre coûte l'équipe entière, gagner ne coûte rien, la réserve prend la suite — **907 ‰ à neuf objets pour 3,69 brûlés**, 634 ‰ à six, 230 ‰ à trois : le coffre devient la progression, et le puits se chiffre. Puis, sur la question de l'auteur (« pas trop punir ? »), **(e) perdue-1** — une défaite ne coûte que la première tombée : sur six objets **992–995 ‰ pour 1,27 objet par run**, 5 ‰ perdent tout ; sur trois, 460 ‰ (l'équipe se bat à deux, puis à une). Le nombre d'objets par défaite est **le** réglage du puits — trois par défaite : 3,69 par run ; un : 1,27 —, rien au moteur. Non mesuré : deux par défaite, la fuite (V3), le revenu d'objets. **À l'auteur, avant PR 5b : l'appétit du puits** |
...
| **A26** | `localcontext()` maintenant (réinitialisation) ou à la prochaine ? | `HANDOVER_HYGIENE_P6.md` §5 | **TRANCHÉ PAR L'AUTEUR le 2026-09-14 : à la prochaine réinitialisation, jamais seul** — **FAIT le 2026-09-15** avec `eidos-testnet-4` |
| **A27** | `format_chaine` : corrigé seul, ou avec un lecteur qui refuse ? | idem | **FAIT le 2026-09-15** : un lecteur (`noeud.config()`, 2 contrôles) |
...
**PR 5a livrée le 2026-09-14** (`banc-veillee`) : `atelier/scripts/banc-veillee.ts` (`npm run banc-veillee`, 1 000 runs × 12 configurations en ≈ 14 min ; 24 runs en CI, 9 contrôles), relu par trois lentilles adversariales (13 constats tenus, tous corrigés : la dernière salle ne se joue pas, la classe est lue du mot, les douze configurations appariées, l'arbre nu est une fin, chaque bataille dans une case, un roster mort n'arrive pas). **A18 tranché par la mesure (9 salles)** ; **A17 rendu à l'auteur comme un choix de butin** (23 ou 9 feuilles) ; **A5 mesuré, A28 posé** (la permadeath telle qu'écrite vide le roster à la salle 1). Chiffres dans `BIBLE_VEILLEE.md` §4.5 et §1.1 ci-dessus.

**PR 5a bis livrée le 2026-09-15** (`banc-a28`) : les contreparties d'A28 au même banc, **21 configurations de plus** (les trois socles à 9 salles en permadeath sous « perdue », « recrue », une réserve de 6, 9 ou 12, puis « perdue » sur une réserve de 6 ou 9 — 33 en tout, ≈ 35 min, un contrôle de plus qui les lit run par run : sans défaite « perdue » rejoue le socle, la réserve prolonge le roster sans le changer, la recrue est l'Indéchiffré relevé, une feuille chacune). **Aucune ne touche au moteur, aucune ne prend le seuil de la bible sans une réserve** : « perdue » seule 230 ‰, la recrue 120 ‰ à son plafond (chantier tué), la réserve seule 818 ‰ à douze objets pour 8,45 brûlés par run, **« perdue » sur une réserve de neuf 907 ‰ pour 3,69** — recommandée, rendue à l'auteur. Tableau et lecture dans `BIBLE_VEILLEE.md` §4.5.

**Le cran en dessous, mesuré le même jour** sur la question de l'auteur (« perdre coûte l'équipe entière, n'est-ce pas trop punir ? ») : **« perdue-1 »**, une défaite ne coûte que la première tombée (lue au journal du moteur, riposte comprise), six configurations de plus (39 en tout, ≈ 40 min, un contrôle de plus). Sur six objets, **992–995 ‰ arrivent pour 1,27 objet par run** et 5 ‰ perdent tout ; sur trois, 458–461 ‰ — l'équipe amputée perd 287 ‰ de ses batailles au lieu de 160. Le nombre d'objets par défaite est le réglage du puits (trois : 3,69 par run ; un : 1,27) ; « deux » n'est pas mesuré. Rendu à l'auteur : l'appétit du puits.

**Le bot qui ramasse, mesuré le même jour** sur la question suivante de l'auteur (« sous « une mort à 32 », le joueur a-t-il plus de chances et de possibilités de finir ? ») : six configurations de plus (45 en tout, ≈ 50 min, un contrôle de plus), un ou deux gestes de butin par salle gagnée sous une garde d'une feuille par salle à franchir, rien de réservé pour les batailles. **Non** : à un geste par salle, « une mort à 32 » refuse un geste dans 168 ‰ des runs et se vide dans 105 ‰ ; à deux, elle se vide dans 658 ‰ ; « un coup à 64 » finance deux gestes par salle (930 ‰ arrivent, 11 feuilles restantes). La force de « une mort » n'est pas la marge, c'est la lisibilité du prix. A17 reste à l'auteur, avec ce chiffre de plus ; recommandation : « un coup à 64 ».

**PR 5b livrée le 2026-09-16** (`veillee-bataille`) : la bataille entre dans la Veillée. `eidos-veillee/2`, cinquième geste **frapper** (`argFrapper` : attaquant × 256 + cible ; un coup n'est jamais du butin, `jugerVeillee` et `lectureVeillee` rendent `coups`) ; `/1` n'a plus de lecteur. `veillee-tour.ts` : une salle dont un occupant reste est **tenue** — parler, creuser, ouvrir y sont refusés (`tenue`), **franchir passe, prendre aussi** (la capsule est l'autre façon de vider une salle : le handover l'avait listé parmi les refus, mais un abattu ne se prend pas et la capsule n'aurait plus jamais servi) ; `ouvrirBatailleDansCoffre` (un à trois objets, les occupants restants en face, les feuilles de l'arbre, rien de signé), `jouerDansBataille` (se déplacer et passer gratuits ; **frapper : l'acte d'abord, la feuille ensuite**, réserve d'indice avant), `passerLaMainDansBataille` (gratuit) ; la bataille vit dans la jauge (`Tour.veillee.bataille` : étape, étage, indices, feuilles à l'ouverture, actes dont « main » ; `batailleDansCoffre` la **rejoue depuis l'ouverture** à chaque lecture et rend null si le coffre a changé sous elle, si un acte ne se rejoue pas ou si les feuilles ne suivent plus) ; victoire → `tour.abattus` (un mort ne se prend ni ne se combat deux fois, `occupantsRestants` les retire) ; défaite **ancrée** → la première tombée, riposte comprise (`premiereTombee`), quitte le coffre (A28) ; libre → rien ; épuisé → la veillée l'est aussi, le sac est perdu. Une bataille ouverte se finit d'abord (franchir et le butin refusés, `bataille`) ; **enlisée** après `TOURS_MAX` passages de main sans issue, franchir passe et rien n'est perdu (le bot en voit une à deux par run de huit) ; s'effacer en pleine bataille la compte perdue. Le store (`batailleOuvrir`, `batailleJouer`, `bataillePasser`), `BatailleView` (joue la bataille de la veillée quand il y en a une, la lecture depuis la jauge, la sélection à la page), `VeilleeView` (l'état de la salle, le lien vers la bataille, les boutons tenus). **Le bot se bat** (trois captures, la politique du dépôt des deux côtés) : sur 20 runs, l'avare garde 25,2 feuilles (le banc `coup-64-9` : 23, +9,6 %), le gourmand 16,6 avec 6,9 de butin (`coup-64-9-b1` : 17 et 6,6, −2 %) — dans les ±10 % demandés. Contrôles : `veillee.test.ts` +3 (argument du coup, coup jugé à part du butin, `/1` refusé), `veillee-tour.test.ts` +6 (salle tenue, l'acte d'abord, la jauge relue à la trace près et absurde → null, victoire, défaite ancrée/libre, épuisé/enlisée/effacement) ; 634 tests, `npm run langue` 0/885 ; labo `aura_veillee.py` lit le cinquième geste, fixtures regénérées (la jouée : la bataille de la porte, une bêche, trois franchir, abandon). Reste PR 6 : la bataille rejouée par le juge dans la preuve.

**Décisions de l'auteur du 2026-09-15**, sur ces chiffres : **A17 = un coup, arbre de 64** (D2 confirmée ; l'équilibre tactique de R2, mesuré à 64 feuilles par bataille, tient dans la run : le plafond ne mord que dans 7 ‰ des batailles à deux gestes de butin par salle) ; **A28 = la première tombée** (une bataille perdue retire du coffre la première unité tombée, rien à la victoire ni au nul, le coffre tel qu'il est pour réserve). Le format de la preuve passe à `eidos-veillee/2` avec le geste de combat, l'arbre reste à 64 : **PR 5b peut s'ouvrir** (handover §4).
...
**PR D — la fiche d'objet, FAITE le 2026-09-15** (`fiche-objet-sans-jargon`), sur la remarque de l'auteur (« verbeux, trop détaillé, trop de jargon ») : la fiche vivait dans `objets-lexique.ts` et `fiche.ts`, hors `i18n.ts`, donc hors de `npm run langue` — elle était passée entre les mailles. Réécrite sous la règle : **trois registres** (forme, caractère, traits) au lieu de quatre, **le bloc « Technique » disparaît** (mot en hexadécimal, quaternion, forme canonique, composante omise, nonce, palier, alignement à l'ancre, « tenue −635 353 », rang et cellule du catalogue, proximité /100), il reste un pied « Bloc 1234 · sceau ○✚✚ » ; les 21 caractères commencent au pronom (la forme vient de nommer la classe et le régime), les pierres disent « se place avant/après la pièce » et non « T·q en préfixe », les âges « le premier, le plus long », l'accord du coffre sans somme ; « de Uranie » → « d'Uranie » (`titres.ts`). La `Fiche` (donnée) ne change pas : `ficheDe` garde tout pour les contrôles et le forum. **Contrôle nouveau** : le lexique entier et chaque fiche rendue (100 formes × 4 âges, avec pierre, gemme et deux voisins ; ~3 500 phrases FR/EN) passent `manquements()` — un mot banni ou une phrase de plus de 25 mots casse le test, et un second contrôle exige l'absence du jargon retiré. Atelier 623 → 624 tests.
...
**Cible.** Point fixe **s = 0,1115 IONOS/joueur/jour**, soit **1 076 joueurs** portés par le budget actuel et 8 609 sur l'émission entière ; et, contrainte dure, **la somme des transactions dédiées reste sous 192 par jour**.
...
| **Le nom donné une fois** — brûler pour nommer, à 10 000 atomes | rien ne se croit | les **255 étages** de la Tour se nomment pour **0,0255 IONOS** en **une** transaction ; la table de titres entière pour 2,27 IONOS ; et 226 800 sorties de poussière seraient rehachées à chaque bloc, à jamais |
...
| **C4 PR 5a, le banc des budgets** (2026-09-14) | Aucune constante, aucun fichier du jeu : un banc sans signature rejoue des runs entières sous douze configurations. **27 salles ne se financent sous aucune règle** (A18 tranché : 9) ; à 9 salles, « un coup » (64) laisse 23 feuilles de butin et « une mort » (32) en laisse 9 — A17 est un choix d'appétit, rendu à l'auteur ; **la permadeath telle qu'écrite vide le roster à la salle 1** (A28). Trois lentilles adversariales, 13 constats corrigés avant la mesure définitive. Atelier 612 → 621 tests |
...
| **C4 PR 5a bis, les contreparties de la permadeath** (2026-09-15) | Le même banc, 21 configurations de plus à 9 salles, appariées aux douze du 14 ; aucune ne touche au moteur. **La recrue est morte à son plafond** (120 ‰ d'arrivées, capsule gratuite et capture sûre) ; **« perdue » seule** ramène la permadeath à « une défaite coûte l'équipe » et s'arrête à 230 ‰ ; **la réserve seule** brûle 8,45 objets par run pour 818 ‰ ; **« perdue » sur une réserve de neuf** arrive à 907 ‰ pour 3,69 objets — le coffre devient la progression (230 / 634 / 907 ‰ à trois, six, neuf objets) et le puits se chiffre. Puis, sur la question de l'auteur, **le cran en dessous** : une défaite ne coûte que la première tombée — sur six objets, **995 ‰ pour 1,27 objet par run** ; le nombre d'objets par défaite est le réglage du puits. Rendu à l'auteur (A28). Puis, sur la question suivante, **le bot qui ramasse** (A17) : « une mort à 32 » finance un geste de butin par salle, pas toujours (168 ‰ de refus, 105 ‰ d'arbres vides) ; « un coup à 64 » en finance deux (930 ‰, 11 feuilles restantes) — la force de « une mort » est la lisibilité du prix, pas la marge. Atelier 621 → 624 tests |
| **testnet-4 : l'unité devient l'ionos** (2026-09-15) | Décision d'auteur : la monnaie d'Eidos s'appelle l'**ionos** (ἰόν, ce qui va ; ticker `IONOS`, 1 ionos = 10⁸ atomes) — « eidôlon » et `EIDL` retournent à Eidolon, l'autre pièce de l'écosystème, avec laquelle ils entraient en collision (`EIDL` y est déjà un préfixe de protocole). `genesis.json` regénéré (`monnaie`, `a_ionos`, `total_ionos` ; tables et bloc 0 **inchangés**, seules les empreintes de `genesis.json` et d'`eonis.py` bougent), `eonis.py` passe en `localcontext()` (H1, A26) sans qu'un atome des tables ne change, `noeud.py` lit `format_chaine` et refuse l'écart (A27, `_test_config` 2 contrôles), `GRAINE` `eidos-testnet-4`, sept clés neuves, chaîne et état du testnet-3 retirés (206 blocs, un compte servi). Les étiquettes hachées `eidos-*`, les formats nommés et `lib/eidos/` restent : ce sont des identifiants de protocole, pas des noms ; le témoin Eidolon n'a rien à suivre. Le type `Ionos` de `relique.ts` (la lumière d'une relique) devient `Lumis`. Le jeu — la Tour, la Veillée, la bataille — s'appelle **la Tour** (décision d'auteur du 2026-09-16 ; « Anodos », ἄνοδος, proposé la veille, écarté) : la Veillée en est une nuit, la bataille un combat. Posé par la PR « nom-la-tour » dans le Guide (`guide.jouer`, `guide.10p`, `guide.11p`), l'écosystème (`eco.tour`, `eco.veillee`), les README et `CLAUDE.md` §1 — aucune clé neuve, le cliquet de la langue à zéro |
| **l'émission ne compte pas les joueurs** (2026-09-15, soir) | Question de l'auteur : le jeu doit faire boule de neige avec ses joueurs — l'émission doit-elle suivre ? Tranché (a) : la table reste telle quelle — `R(h)` en cosinus, quatre âges 40/30/20/10 sur 832/624/416/208 cycles (le rapport 4:3:2:1 des yugas), 62 899 200 ionos, aucune dépendance au nombre de joueurs et l'ordre des âges gardé (Satya d'abord : 53 % du total, Kali 3 %). Motif : la chaîne ne sait pas compter les joueurs (`SPEC_SYBIL.md` : une adresse est gratuite), donc un pot qui grossirait avec eux grossirait avec les machines ; la boule de neige est dans la **répartition** d'une masse bornée (la part rétrécit, ce qu'elle vaut et ce qu'on en fait grossissent), jamais dans la source. L'inversion des âges (10 → 40, une émission croissante, bornée, sans rien compter) a été posée avec ses chiffres et refusée. Même jour : `chaine-reseau.test.ts` altère au plus profond disponible (`min(3, n−1)`, `min(5, n−1)`) au lieu des blocs 3 et 5, sinon il restait rouge cinq heures après chaque réinitialisation |
| **neuf salles** (2026-09-16) | A18 confirmé par l'auteur et codé (PR « neuf-salles ») : `ETAGES_PAR_BANDE` 1, `ETAPES` 9, `FRANCHIR_AU_SOMMET` 8, `SAC_PLACES = DALLE_N²`. En codant : le cran seul n'offrait que 73 étages sur 255 ; `etageDe(i, p, h)` prend le hachage de la transition (le cran choisit le neuvième de la bande, l'octet h[1] l'étage dedans), 227 étages joignables, phase 0 tenue (227 / 0,139 / 0,119). Tables gelées régénérées (pendule, bot, échantillon du banc), six configurations du banc rejouées à 1 000 runs : mêmes verdicts qu'A17 et A28. Labo : `unification.py`, `pendule9_run.py` (K17 : 9), `aura_veillee.py`, quatre fixtures regénérées, 51 PASS. Textes : 9 salles, 8 franchir, sac de 81, FR/EN, `npm run langue` 0. Sans bataille, la Tour seule ne vide plus l'arbre (≤ 47 feuilles) : le dilemme de la dernière feuille arrive avec 5b. 625 tests |
| **C4 PR 5b, la bataille dans la Veillée** (2026-09-16) | Voir C4 ci-dessus. Deux écarts au handover, dits : prendre passe dans une salle tenue (sinon la capsule ne servait plus), et une bataille enlisée (`TOURS_MAX` mains) se quitte en franchissant. Deux règles neuves avec leur contrôle : une bataille ouverte se finit d'abord ; s'effacer en pleine bataille la compte perdue. Le bot se bat : ±10 % du banc tenus. 634 tests |
