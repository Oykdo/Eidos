...
**PR B — Vérifier et Lire** : coffre, journal, témoin, signes, carte, lectures, robinet, hub — réécriture FR/EN, une idée par phrase, chaque phrase citant une règle vraie du code (CLAUDE.md §8) ; capture headless de chaque page. Cible : le cliquet à zéro sur ces pages.
**PR C — Jouer et le Guide** : tour, veillée, bataille, coffre de l'heure, reliques, guide. Cible : **cliquet à 0 / 0 / 0 / 0**, Guide ≤ 8 000 caractères, le vocabulaire de chaîne relégué à une section « Pour les curieux ». Les libellés gelés d'`i18n.test.ts` (reliques, `guide.09p`) sont à regeler avec les nouveaux textes.
