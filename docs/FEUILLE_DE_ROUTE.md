...
| Le budget d'une run entière, sous chaque règle | **1 000 runs × 12 configurations** (coup 64, mort 32, mort 64 × 27 ou 9 salles × roster qui revient ou permadeath), sans signature, `ia.ts` des deux côtés, configurations appariées ; 27 salles : **0 ‰** d'arrivés sous toute règle ; 9 salles : 997–1 000 ‰, avec **23** feuilles de butin (« un coup », 64) ou **9** (« une mort », 32) ; permadeath telle qu'écrite : roster balayé à la salle 1 dans 996–1 000 ‰ des runs ; échantillon de 24 runs en CI en ~15 s | `atelier/scripts/banc-veillee.ts` (`npm run banc-veillee`), `banc-veillee.test.ts` |
...
| **A28** | la permadeath vide le roster (A5 mesuré) : quelle contrepartie ? (a) une unité tombée ne compte que dans une bataille **perdue** ; (b) le recrutement en cours de run — les captures se battent ; (c) assumer des runs de deux salles | `BIBLE_VEILLEE.md` §4.5 (5), V5 | l'auteur, avant PR 5b ; (a) et (b) se mesurent au banc 5a en une configuration de plus chacune |
