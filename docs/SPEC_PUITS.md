...
La rareté d'Eidos n'est pas la monnaie : c'est **192 transactions par jour** (`MAX_ENVOIS = 8`) et **120 IONOS par jour** (`a·T/8`). Un puits granulaire — un micro-paiement par craft, par sertissure, par fantôme — **ne rentre pas dans les blocs**. Les seuls puits possibles sont **gros et rares**, ou **gratuits en place** (une sortie de plus, 28 o contre 2 283 o pour une transaction : **81,5×**).
Trois puits suffisent : la **mise du sceau** (0,0625 IONOS/joueur/jour), le **péage de lignée** (0,025), le **nom** (0,016). Point fixe à **s = 0,1115 IONOS/joueur/jour**, soit **1 076 joueurs** sur le budget actuel et **8 609** sur l'émission entière.
...
L'émission est bornée : 62 899 200 IONOS, 2 096 640 blocs, **239,2 ans** à un bloc par heure. La masse ne peut pas exploser. Ce qui explose est le **rapport d'ancienneté**, et il suffit à fermer le marché de D6.
...
À `g = 0,048` (le maximum soutenable pour N = 1 000, §4) : **43,5 IONOS** après un an, **130** après trois, **435** après dix. Le robinet donne **1 IONOS par époque** à un nouveau venu. Rapport vétéran/nouveau : **43×, 130×, 435×** — et il ne plafonne jamais.

Or les objets, eux, **sont bornés** : le sac plafonne à 27 (`SPEC_COFFRE_HORAIRE.md` K44), la permadeath en brûle, la forge en consomme deux par craft. Une masse qui croît linéairement en face d'un stock borné donne un prix qui croît linéairement : **le prix d'un T9 suit la poche du vétéran**. Au bout de trois ans, l'objet coûte 130 IONOS, le nouveau venu en a 1, et le marché se ferme. C'est exactement le mécanisme d'Axie, transposé : non pas « la monnaie ne vaut plus rien », mais **« le ticket d'entrée tend vers l'infini »**.
...
Mesuré à la hauteur 45 : 944,27 IONOS émis, 47 sorties, `invariant true`. L'émission moyenne en Satya vaut **40 IONOS/bloc = 960 IONOS/jour** ; le budget du robinet vaut `a·T/8` = **5 040 IONOS/époque = 120 IONOS/jour**. Besoin d'une moisson à **1 IONOS par 12 h**, comparé à l'émission cumulée :

| blocs | émis (IONOS) | N=10 | N=100 | N=1 000 | N=10 000 |
|---|---|---|---|---|---|
...
En gras : au-delà de ce que la chaîne a émis. **1 IONOS par 12 h est impossible dès mille joueurs**, avant même de parler de puits. La moisson ne peut pas être ronde ; elle doit être une fraction.
...
- absorber 120 IONOS/jour avec 192 transactions exige **≥ 0,625 IONOS par transaction** ; absorber 960 IONOS/jour en exige **5,00** ;
...
**Un puits granulaire est physiquement impossible.** Payer 1/64 d'IONOS par craft, par sertissure, par fantôme inscrit : il n'y a pas la place. Les puits doivent être **gros et rares**, ou **portés par une transaction qui existait déjà**.
...
| # | puits | ce qu'il achète | prix | où vont les atomes | fréquence | tx dédiée | débit IONOS/j/joueur | pouvoir | verdict |
|---|---|---|---|---|---|---|---|---|---|
| **P1** | **Mise du sceau** | qu'un acte **compte** : bataille ancrée, veillée, cohorte | **4 IONOS / 64 actes** = 1/16 par acte | `A(sceau)`, brûlé | 1 acte/jour (D4-c), donc 1 mise / 64 j | 0,0156 | **0,0625** | non — achète une trace, pas une stat | **retenu, premier** |
| **P2** | **Péage de lignée** | un maillon `eidos-objet/1` valide | **1/4 IONOS** (`ATOMES/4`) | `A(lignee)`, sortie de plus **dans la transaction d'échange** (28 o) | 0,1 échange/jour | **0** | **0,025** | non — taxe le transfert | **retenu** |
| **P3** | **Nom** | nommer une unité, une relique, un fantôme, une lignée | **2 IONOS** | `A(nom) = sha256("eidos-nom/1" ‖ nom)[:20]` — le nom **est** l'adresse | 0,008/jour | 0,008 | **0,016** | **zéro** | **retenu** |
| **P4** | **Porte de quartier** | franchir 64 / 128 / 192 | **4 / 8 / 16 IONOS** | `A(porte)`, brûlé | 3 fois dans une vie | 0,0015 | **0,008** | indirect (butin de zone) — gelé jusqu'à R2 | **retenu, gelé** |
...
| P6 | Sertissure de plus | une alvéole au-delà de `SOCKETS_MAX = 2` | 1/2 IONOS | `A(forge)` | rare | 0,01 | 0,005 | non, **mesuré** : la 3ᵉ pièce vaut +0,9 pt de plafond, la 4ᵉ +0,06 | **écarté** : personne ne paiera 0,5 IONOS pour +0,9 % |
| P7 | Classement / cohorte | inscrire un fantôme | 1/32 IONOS | `A(sceau)` | 0,05/jour | 0,05 | 0,0016 | non | **fondu dans P1** : trop granulaire seul |
...
**Barème retenu**, `s = 0,1115 IONOS/joueur/jour` → **g = 0,0439**, arrondi à **g = 1/24 IONOS par 12 h** (0,04167) :

| N | robinet (IONOS/j) | g max budget `a·T/8` | g max émission | g max **capacité** | g retenu | s requis | tx/jour | `MAX_ENVOIS` requis |
|---|---|---|---|---|---|---|---|---|
...
- **N < 331 : c'est la capacité d'absorption qui borne, pas l'argent.** Un joueur ne peut pas ancrer plus de ~5 batailles par jour ; à 1/16 IONOS l'acte, il ne brûle pas plus de **0,363 IONOS/jour** quoi qu'il possède. Verser davantage s'accumule, point. Le budget autoriserait `g = 5,99` à N = 10 ; **le jeu n'en absorbe que 0,170**.
- **N > 331 : c'est le trésor qui borne.** À N = 10 000, le robinet seul (238 IONOS/j) dépasse le budget `a·T/8` (120) : il faut porter `BUDGET_RATIO` de 8 à 1, et `MAX_ENVOIS` de 8 à 53 (640 Mo/an de chaîne).
...
**Masse en circulation à l'équilibre.** Un joueur garde le tampon de sa dépense la plus grosse : ~1 à 4 IONOS (une mise, un nom). `M* ≈ N × 1 à 4 IONOS`, soit **1 000 à 4 000 IONOS pour N = 1 000** — contre **350 000** au bout de dix ans sans puits. Rapport **100 à 350×**.
...
1. **P1 est-il payé par acte ou par mise ?** La mise (4 IONOS / 64 actes) est la seule qui tienne dans 192 tx/jour, au prix d'un décompte hors chaîne dans `batailles/index.json`. *Recommandation : la mise, en disant clairement que le brûlage est la preuve et le décompte une figure.*
2. **Le péage de lignée est-il obligatoire ou facultatif ?** Obligatoire : le juge refuse un maillon dont la transaction ne porte pas de sortie ≥ 1/4 IONOS vers `A(lignee)`. Facultatif, il n'absorbe rien. *Recommandation : obligatoire, et gratuit pour le **premier** maillon (l'origine), pour ne pas taxer le coffre horaire.*
