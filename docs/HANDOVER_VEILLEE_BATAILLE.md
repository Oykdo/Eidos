...
**Dépôt :** Oykdo/Eidos · **Rédigé le :** 2026-09-14, contre `9927508` (612 tests, 127 suites) · **Statut :** **PR 5a livrée le 2026-09-14** (le banc, §3 mesuré), **5a bis le 2026-09-15** (les contreparties d'A28 mesurées, §3) ; A18 tranché par la mesure, A17 et A28 attendent l'auteur, chacun avec ses chiffres ; 5b et 6 restent à coder
...
**Mesuré le 2026-09-15 — les contreparties d'A28** (`npm run banc-veillee`, 21 configurations de plus : les trois socles à 9 salles en permadeath, appariées aux douze du 14 ; tableau et lecture dans `BIBLE_VEILLEE.md` §4.5). Aucune ne touche au moteur :
...
- Non mesuré : une défaite qui ne coûterait qu'une unité (règle neuve, une configuration si l'auteur la veut), la fuite (V3), une K.O. qui reviendrait blessée (la tenue repart de `ecu` à chaque salle, D1).
...
|---|---|---|---|
| **A17** | une feuille signe un coup, ou une mort ? | **mesuré** : les deux arrivent à 9 salles ; « un coup » (64) laisse 23 feuilles de butin, « une mort » (32) en laisse 9 — un choix d'appétit ; ma recommandation : **« un coup »**, déjà tranchée (D2), rien au moteur, et 23 feuilles pour 9 salles laissent vivre l'hôte, la bêche et la capsule | **l'auteur**, sur les chiffres |
...
| **A28** | la contrepartie de la permadeath : (a) une unité tombée ne compte que dans une bataille **perdue** ; (b) les captures se battent (recrutement en cours de run) ; (c) assumer des runs de deux salles ; (d) le coffre porte une réserve | **mesuré le 2026-09-15** (§3) : (a) seule 230 ‰, (b) 120 ‰ à son plafond — chantier tué —, (d) seule 818 ‰ à douze objets pour 8,45 brûlés par run ; **(a) + (d)** : perdre coûte l'équipe, gagner rien, la réserve prend la suite — **907 ‰ à neuf objets pour 3,69 brûlés**, 634 ‰ à six. En termes de joueur : neuf runs sur dix vont au bout, un run coûte moins de quatre objets, un sur onze perd tout. Ma recommandation : (a) + (d), le coffre tel qu'il est ; en 5b, `Tour.veillee.bataille` retire la lice entière à la défaite et rien à la victoire | **l'auteur**, avant 5b, sur les chiffres |
...
```bash
cd atelier && npm run typecheck && npm test        # 622 aujourd'hui, 0 échec
npm run banc-veillee                                # 5a et 5a bis : les trente-trois configurations, hors CI (≈ 35 min)
