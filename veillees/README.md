...
- **Une ligne dans `index.json`** (un tableau de noms de fichiers), ajoutée à
  la fin, par une **pull request**. L'ordre du dépôt est public : c'est lui qui
  départage deux preuves valides sur la même pièce le même jour — la première
  inscrite est classée, l'autre est un doublon.
...
bible §4.1) n'est pas encore distinguée : le juge accepte toute tête signée.
