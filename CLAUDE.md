...
atelier/            interface web (TanStack Start, React), rejoue la spec en TS ;
                    ...
                    scripts/banc-veillee.ts = le budget d'une run entière sous chaque règle (A17, A18, A5, A28), 39 configurations, sans signature
...
cd atelier && npm ci && npm run typecheck && npm test && npm run build   # 623 tests Eidos
...
npm run banc-veillee           # le budget d'une run : 1 000 runs × 39 configurations (~40 min), le juge d'A17, d'A18 et d'A28
```
...
- Coffre horaire — un coffre par bloc, une pièce par claim, neuf tiers géométriques — PR 1 FAITE (2026-09-07) ; l'ouverture (`<dialog>` natif, objets neufs surlignés à l'inventaire, A15 tranché : pris entier, rien de perdu) FAITE (2026-09-13), voir docs/SPEC_COFFRE_HORAIRE.md
