...
atelier/            interface web (TanStack Start, React), rejoue la spec en TS ;
                    ...
                    scripts/banc-veillee.ts = le budget d'une run entière sous chaque règle (A17, A18, A5, A28), 33 configurations, sans signature
...
cd atelier && npm ci && npm run typecheck && npm test && npm run build   # 622 tests Eidos
...
npm run banc-veillee           # le budget d'une run : 1 000 runs × 33 configurations (~35 min), le juge d'A17, d'A18 et d'A28
```
