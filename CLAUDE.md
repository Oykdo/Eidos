...
`envoi.ts`, `wots.ts`, `xmss.ts`, `relique-qr.ts`, `pendule.ts`, `ancrage.ts`, `veillee.ts`, `veillee-tour.ts`, `fantomes.ts` — chacun avec son `.test.ts` ;
...
- La Veillée — roguelike XMSS, PR 1 FAITE, PR 2 en cours (jauge et gestes reliés à la Tour faits le 2026-09-07), voir docs/BIBLE_VEILLEE.md
