...
cd atelier && npm ci && npm test # 30 script tests and 623 Eidos tests, vectors shared with Python
```
...
| `atelier/` | — | web atelier, tactical engine included (`src/lib/eidos/tactique/`); `npm test` runs 30 script tests and 623 Eidos tests | 623 |
...
npm run banc-veillee           # the leaf budget of a whole run under each rule: 1,000 runs × 39 configurations, ~40 min
