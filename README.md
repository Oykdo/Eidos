...
cd atelier && npm ci && npm test # 30 script tests and 625 Eidos tests, vectors shared with Python
```
...
| | **Vigil** | The daily run: 64 WOTS+ leaves as life, 9 rooms from the day's first block, the bag, the tree of leaves on screen, the proof, the leaderboard and the ghosts |
...
| `atelier/` | — | web atelier, tactical engine included (`src/lib/eidos/tactique/`); `npm test` runs 30 script tests and 625 Eidos tests | 625 |
