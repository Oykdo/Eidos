...
cd atelier && npm ci && npm test # 30 script tests and 621 Eidos tests, vectors shared with Python
```
...
| `atelier/` | — | web atelier, tactical engine included (`src/lib/eidos/tactique/`); `npm test` runs 30 script tests and 621 Eidos tests | 621 |
...
npm run banc-veillee           # the leaf budget of a whole run under each rule: 1,000 runs × 12 configurations, ~14 min
