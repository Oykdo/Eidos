...
5. Keep a Vigil: sixty-four one-time keys as life, the day's nine rooms, a bag that the summit pours into the vault and that the last leaf loses.
...
cd atelier && npm ci && npm test # 30 script tests and 634 Eidos tests, vectors shared with Python
```
...
| | **Vigil** | The daily run: 64 WOTS+ leaves as life, 9 rooms from the day's first block, the held room and its battle (one blow, one leaf), the bag, the tree of leaves on screen, the proof, the leaderboard and the ghosts |
...
| `veillees/` | — | deposited vigil proofs (`index.json`, one `eidos-veillee/2` file per proof), judged in every browser, never by a server | — |
| `docs/` | — | specifications, the vigil bible, the roadmap, the lore; banner generator | 2 |
| `atelier/` | — | web atelier, tactical engine included (`src/lib/eidos/tactique/`); `npm test` runs 30 script tests and 634 Eidos tests | 634 |
...
npm run veillee-bot 60         # the vigil bot: three policies that fight, a reading of the leaf budget
npm run banc-veillee           # the leaf budget of a whole run under each rule: 1,000 runs × 45 configurations, ~50 min
npm run banc-r2                # the price of the axes (R2) on the real engine and policy: 440,320 duels, ~16 min
