...
cd atelier && npm ci && npm test # 30 script tests and 622 Eidos tests, vectors shared with Python
```
...
| `atelier/` | — | web atelier, tactical engine included (`src/lib/eidos/tactique/`); `npm test` runs 30 script tests and 622 Eidos tests | 622 |
...
npm run banc-veillee           # the leaf budget of a whole run under each rule: 1,000 runs × 33 configurations, ~35 min
